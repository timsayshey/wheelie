<cfoutput>
	...
	<cfset contentFor(headerTitle	= '<span class="fa fa-pencil"></span> Posts')>
	<cfset contentFor(headerButtons = 
				'<li class="headertab">
					#linkTo(
						text		= '<span class="fa fa-pencil"></span> Add Post',
						route		= "admin~Action",
						module		= "admin",
						controller	= "posts",
						action		= "new", 
	...
	<div class="row-regular">
		...
		#startFormTag(route="admin~Action", module="admin", controller="posts", action="deleteSelection", enctype="multipart/form-data")#	
			...
			<div class="btn-group pull-right">
				<a href='#urlFor(route="admin~Action", module="admin", controller="posts", action="index")#?display=grid' class="btn btn-default #gridActive#"><span class="fa fa-th-large"></span></a>
				<a href='#urlFor(route="admin~Action", module="admin", controller="posts", action="index")#?display=list' class="btn btn-default #listActive#"><span class="fa fa-th-list"></span></a>
			</div>
			...
			<div id="post" class="col-md-12">
				...
				<cfloop query="qPosts" startrow="#pagination.getStartRow()#" endrow="#pagination.getendrow()#">		
					...
					<cfsavecontent variable="tags">
						<cfif qPosts.id eq homeid><span class="fa fa-home color-danger" title="Homepost"></span></cfif>						
						<cfif qPosts.status eq "published">
							<span class="fa fa-eye-open color-info" title="Published"></span>
						<cfelse>						
							<span class="fa fa-eye-close color-danger" title="#capitalize(qPosts.status)#"></span>
						</cfif>
					</cfsavecontent> 
