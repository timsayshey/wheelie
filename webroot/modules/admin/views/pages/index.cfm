<cfoutput>
	...
	<cfset contentFor(headerTitle	= '<span class="fa fa-file-new"></span> Pages')>
	<cfset contentFor(headerButtons = 
				'<li class="headertab">
					#linkTo(
						text		= '<span class="fa fa-file-new"></span> Add Page',
						route		= "admin~Action",
						module	= "admin",
						controller	= "pages",
						action		= "new", 
	...
	<div class="row-regular">
		...
		#startFormTag(route="admin~Action", module="admin", controller="pages", action="deleteSelection", enctype="multipart/form-data")#	
			...
			<div class="btn-group pull-right">
				<a href='#urlFor(route="admin~Action", module="admin", controller="pages", action="index")#?display=grid' class="btn btn-default #gridActive#"><span class="fa fa-th-large"></span></a>
				<a href='#urlFor(route="admin~Action", module="admin", controller="pages", action="index")#?display=list' class="btn btn-default #listActive#"><span class="fa fa-th-list"></span></a>
			</div>
			...
			<div id="page" class="col-md-12">
				...
				<cfloop query="qPages" startrow="#pagination.getStartRow()#" endrow="#pagination.getendrow()#">		
					...
					<cfsavecontent variable="tags">
						<cfif qPages.id eq homeid><span class="fa fa-home color-danger" title="Homepage"></span></cfif>						
						<cfif qPages.status eq "published">
							<span class="fa fa-eye-open color-info" title="Published"></span>
						<cfelse>						
							<span class="fa fa-eye-close color-danger" title="#capitalize(qPages.status)#"></span>
						</cfif>
					</cfsavecontent> 
					...
					<cfsavecontent variable="prependBtns">
						<cfif checkPermission("superadmin")>
							#linkTo(									
								text		= '<span class="fa fa-eye-open"></span> View',
